...
public class OpsOrbisScoreboard extends CustomUIHud {
    ...
    public OpsOrbisScoreboard(PlayerRef playerRef, MatchInstance matchInstance) {
        super(playerRef);
    ...
    public void rafraichir() {
        ...
        try {
            Player p = matchInstance.getTeamManager().getJoueurParRef(this.getPlayerRef());
    ...
    private void injecterDonnees(UICommandBuilder builder) {
        RelicManager rm = matchInstance.getRelicManager();
        if (rm == null) return;

        int capturees = rm.getRelicsCapturees();
        int round = matchInstance.getRoundActuel();
        int eq1Score = matchInstance.getTeamManager().getScoreEquipe1();
        int eq2Score = matchInstance.getTeamManager().getScoreEquipe2();

        long tempsRestantSec = matchInstance.getTempsRestantManche();
        ...
        builder.set("#ScoreBleu.TextSpans", Message.join(
            OpsOrbis.get().getLangManager().get("scoreboard_round", "round", round, "max_round", OpsOrbis.get().getConfigManager().getGlobalConfig().getMaxRounds()),
        ...
        builder.set("#ScoreRouge.TextSpans", Message.join(
            OpsOrbis.get().getLangManager().get("scoreboard_team1", "score1", eq1Score),
            OpsOrbis.get().getLangManager().get("scoreboard_team2", "score2", eq2Score)
        ));
        ...
        Player p = matchInstance.getTeamManager().getJoueurParRef(this.getPlayerRef());
        if (p != null) {
            String campTrad = OpsOrbis.get().getLangManager().getRaw(matchInstance.getTeamManager().getCamp(p).getLangKey());
            String teamTrad = matchInstance.getTeamManager().getEquipeAttaquants().contains(p) || matchInstance.getTeamManager().getEquipeDefenseurs().contains(p) 
                ? (matchInstance.getTeamManager().getEquipeAttaquants().contains(p) == matchInstance.getTeamManager().isEquipe1Attaquant() ? OpsOrbis.get().getLangManager().getRaw("team_1_name") : OpsOrbis.get().getLangManager().getRaw("team_2_name"))
